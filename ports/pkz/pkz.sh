...
do_build() {
  ...
  (
    cd $PKG
    tar --sort=name -cf - . | $GZIP -c >$SRC/pkgbin
...
do_resolve() {
  ...
  if [ $resolve_deps == yes ]; then
    (
      cd $zregs
      ls -1 *.gz | cut -d# -f1 | sort -u >$dep.inst
    )
...
while true; do
  case "$1" in
    ...
    -c)
      shift
      test -r "$1" && zconf=$1 || error "no file $1"
      ;;
    -p)
      shift
      test -d "$1" && zpackages=$(
...
test \( $cmd = clean -o $cmd = list \) -a $# -eq 0 && set $(
  cd $zpackages
  ls
)
